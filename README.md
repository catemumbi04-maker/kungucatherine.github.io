# kungucatherine.github.io
<section id="about-me">
  <h2>Emerging Communications and Public Relations Professional</h2>
  <p>
    I am an emerging Communications and Public Relations professional passionate about crafting compelling narratives, engaging diverse audiences, and supporting organizations in achieving meaningful impact. With hands-on experience in content creation, media engagement, and digital communication, I excel at developing strategic messaging that informs, inspires, and drives positive change. I am eager to contribute my creativity, analytical skills, and enthusiasm for communication to dynamic teams and projects, while continuing to grow as a professional in the public relations and communications field.
  </p>
</section>
<section class="experience">

  <div class="job">
    <h3>Assistant editor</h3>
    <p><strong>MUISSA and Nextrend Magazine</strong></p>
    <span class="date">Jan 2023 - Present</span>
    <ul>
      <li>Managing a team of 23 creatives for a readership of 2,000+.</li>
      <li>Increased Instagram engagement by 40% through design innovation.</li>
    </ul>
  </div>

  <div class="job">
    <h3>Public Relations Volunteer</h3>
    <p><strong>Moi University PR Office</strong></p>
    <span class="date">March 2024 - Present</span>
    <ul>
      <li>Managed crisis communication during the 2024 protests.</li>
      <li>Reduced misinformation by 70% through strategic messaging.</li>
    </ul>
  </div>

  <div class="job">
    <h3>Volunteer Usher</h3>
    <p><strong>PRSK Summit, Eldoret</strong></p>
    <span class="date">Dec 2025</span>
    <ul>
      <li>Coordinated logistics and attendee registration.</li>
      <li>Supported event operations for over 300 national PR delegates.</li>
    </ul>
  </div>

</section>
